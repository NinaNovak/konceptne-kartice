...
def vrni_eno_kartico(idkartice):
    ...
    sql = '''
SELECT naslov_kartice AS naslov,
       replace(group_concat(distinct kljucna_beseda.beseda), ",", ", ") AS kljucne,
       replace(group_concat(distinct programsko_orodje_ali_jezik.ime_orodja), ",", ", ") AS orodja,
       ime_PDF_datoteke AS dat,
       ime_datoteke AS dat_orig       
...
def oblak():
    sql = '''
SELECT kljucna_beseda.beseda AS tag,
...
SELECT MAX(id) as trenutna_kartica FROM konceptna_kartica
    ...
    '''
    id_kartice = conn.execute(sql).fetchone()
    return id_kartice
...
def id_zadnje_kljucne_besede():
    ...
    #zato je zadnji id  najvecji
    #klicemo ga z MAX(id)

    sql = '''
SELECT MAX(id) as id_zadnje
FROM kljucna_beseda
...
def kartica_ima_kljucne(id_kartice, seznam_besed):
    ...
    ki pripadajo tej kartici.

    '''
    vse = vrni_vse_kljucne_besede()  #vse kljucne v bazi
    sez_idjev = []  #seznam id-jev kljucnih besed
    sql = '''
INSERT INTO povezovalna_tabela_konceptna_kartica_x_kljucna_beseda (id_konceptne_kartice, id_kljucne_besede)
VALUES (?, ?)
...
# UREJANJE KARTICE
##############################################################################
def spremeni_naslov(id_kartice, novi_naslov_kartice):
    sql = '''
UPDATE konceptna_kartica
SET naslov_kartice = ?
WHERE id = ?
    ...
    '''
    conn.execute(sql, [novi_naslov_kartice,id_kartice])
    conn.commit()
def odvzemi_kljucno(id_kartice, id_kljucne):
    sql = '''
DELETE FROM povezovalna_tabela_konceptna_kartica_x_kljucna_beseda
WHERE id_konceptne_kartice = ?
        AND id_kljucne_besede = ?

    '''
    conn.execute(sql, [id_kartice, id_kljucne])
    conn.commit()
#(dodajanje ključnih besed je z zgornjo funkcijo kartica_ima_kljucne.)
def odvzemi_orodje(id_kartice, id_jezika):
    sql = '''
...
WHERE id_konceptne_kartice = ?
    ...
    '''
    conn.execute(sql, [id_kartice, id_jezika])
    conn.commit()
#(dodajanje orodja je z zgornjo funkcijo kartica_uci_programsko_orodje_jezik.)
def spremeni_opis(id_kartice, novi_opis):
    sql = '''
UPDATE konceptna_kartica
SET kratek_opis = ?
WHERE id = ?
    ...
    '''
    conn.execute(sql, [novi_opis, id_kartice])
    conn.commit()
##############################################################################
# THE END
##############################################################################
